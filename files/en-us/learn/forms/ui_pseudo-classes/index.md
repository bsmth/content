...
```css hidden live-sample___basic-required-optional
body {
  font: 1em sans-serif;
  margin: 20px auto;
}
...
{{EmbedLiveSample("basic-required-optional", "", "400px")}}
...
Screen readers already let their users know when a radio button or checkbox they encounter is checked/selected, so you don't want them to read out another DOM element that indicates selection — that could be confusing. Having a purely visual indicator solves this problem.
...
{{EmbedLiveSample("required-optional-generated", "", "430px")}}
...
Let's go in and look at a simple example of `:valid`/`:invalid`.
...
{{EmbedLiveSample("valid-invalid", "", "420px")}}
...
Let's look at an example that does exactly this.
The following example builds on top of the previous example to provide out-of-range messages for the numeric inputs, as well as saying whether they are required.
...
<form>
  <fieldset>
    ...
    <div>
      <label for="age"> Age (must be 12+): </label>
      <input id="age" max="120" min="12" name="age" required="" type="number" />
      <span> </span>
    </div>
    <div>
      <label for="email">
...
{{EmbedLiveSample("out-of-range", "", "420px")}}
...
The most relevant parts of the CSS example are as follows:
...
{{EmbedLiveSample("enabled-disabled-shipping", "", "540px")}}
...
{{EmbedLiveSample("readonly-confirmation", "", "700px")}}

> [!NOTE]
> You'll probably rarely use `:enabled` and `:read-write` pseudo-classes, given that they describe the default states of input elements.
...
{{EmbedLiveSample("radios-styled", "", "220px")}}
...
<form>
  <fieldset>
    ...
    <p>
      <input checked="" id="banana" name="fruit" type="radio" value="banana" />
...
{{EmbedLiveSample("radios-checked-default", "", "200px")}}
...
{{EmbedLiveSample("radios-checked-indeterminate", "", "200px")}}
